...
classifier = Classifier("TrainingModels/keras_model.h5", "TrainingModels/labels.txt")
...
while True:
    ...
    if len(hands) == 1:  # hand 1
        ...
        divide = list(dict.fromkeys(results))
        combine = ''.join(map(str, divide))
        select = combine.strip('i')
        if 'i' in divide:
            print(select)  '''
    ...
    if len(hands) == 2:  # hand 2
        ...
        if aspectRatio > 1:
            ...
            cv2.rectangle(img, (x - offset, y - offset),
                          (x + w + offset, y + h + offset), (255, 0, 255), 1)
            # cv2.putText(img, ''.join(select), (x, y - 26), cv2.FONT_HERSHEY_COMPLEX, 1.7, (255, 255, 255), 4,
            #            cv2.LINE_AA)
